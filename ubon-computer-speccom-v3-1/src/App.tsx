...
function InventoryTable({ items, onEdit, onDelete }:{ items: Product[]; onEdit:(p:Product)=>void; onDelete:(id:string)=>void; }){
  return (<div className="border rounded-2xl overflow-hidden">
    <div className="grid grid-cols-12 bg-muted/50 px-4 py-2 text-sm font-semibold">
      <div className="col-span-5">สินค้า</div><div className="col-span-2">หมวด</div><div className="col-span-2 text-right">ราคา</div><div className="col-span-1 text-right">สต็อก</div><div className="col-span-2 text-right"></div>
    </div>
...
// ===== Summary =====
function Summary({ build, sortMode, setSortMode, onReset }:{ build: BuildState; sortMode: SortMode; setSortMode:(v:SortMode)=>void; onReset:()=>void; }){
...
const sortLabel = sortMode==='priceAsc' ? 'ราคาต่ำ→สูง' : sortMode==='priceDesc' ? 'ราคาสูง→ต่ำ' : 'ค่าเริ่มต้น';
...
useEffect(() => {
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <Tabs defaultValue="builder" className="space-y-4">
        ...
        <TabsContent value="summary">
          <Summary build={build} sortMode={sortMode} setSortMode={setSortMode} onReset={resetSpec} />
        </TabsContent>
      </Tabs>
  ...
  <DialogContent className="sm:max-w-[720px] relative">
    <button aria-label="Close" onClick={()=>setSearchOpen(false)} className="absolute right-3 top-3 rounded-full h-8 w-8 flex items-center justify-center hover:bg-muted"><X className="w-4 h-4"/></button>

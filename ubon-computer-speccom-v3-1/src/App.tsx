...
import {
  ...
  Power, Monitor as MonitorIcon, Wrench, CaseSensitive,
...
const STORAGE_KEYS = {
  inventory: "ubonspec.inventory.v3_1",
  build: "ubonspec.build.v3_1",
  pricing: "ubonspec.pricing.v3_1",
  required: "ubonspec.required.v3_1",
  filters: "ubonspec.filters.v3_1",
  smartSync: "ubonspec.smartsync.v3_1",
} as const;
...
function Chip({ children, onClick, active=false }:{ children: React.ReactNode; onClick?: ()=>void; active?: boolean }){
  return <Button type="button" onClick={onClick} variant="secondary" className={`h-7 px-3 rounded-full text-xs md:text-sm whitespace-nowrap leading-none ${active? 'bg-primary/90 text-white hover:bg-primary' : ''}`}>{children}</Button>;
}
function ClearChip({ label, onClear }:{ label:string; onClear:()=>void }){
  return <span className="inline-flex items-center gap-1 bg-slate-100 text-slate-700 px-2.5 h-7 rounded-full text-xs whitespace-nowrap">{label}<button className="opacity-70 hover:opacity-100" onClick={onClear} title="ล้าง"><X className="w-3.5 h-3.5"/></button></span>;
}
...
function InventoryTable({ items, onEdit, onDelete }:{ items: Product[]; onEdit:(p:Product)=>void; onDelete:(id:string)=>void; }){
  return (<div className="border rounded-2xl overflow-hidden">
    <div className="grid grid-cols-12 bg-muted/50 px-4 py-2 text-sm font-semibold">
      <div className="col-span-4">สินค้า</div><div className="col-span-2">หมวด</div><div className="col-span-2 text-right">ราคา</div><div className="col-span-1 text-right">สต็อก</div><div className="col-span-3 text-right">จัดการ</div>
    </div>
    <div className="max-h-[360px] overflow-auto">
      {items.map(p=>(<div key={p.id} className="grid grid-cols-12 items-center px-4 py-2 border-t hover:bg-muted/30 text-sm">
        <div className="col-span-4 truncate flex items-center gap-2"><Badge variant="secondary" className="rounded-xl whitespace-nowrap">{categoryIcon[p.category]}<span className="ml-2">{p.category}</span></Badge><span className="font-medium truncate" title={p.name}>{p.name}</span></div>
...
function CategoryAttrFields({ category, attr, setAttr }:{ category: Category; attr: Record<string, any>; setAttr: (a:Record<string,any>)=>void; }){
  ...
  if (category==="RAM"){
    return (<div className="grid grid-cols-2 gap-3">
      <div>
        <Label className="whitespace-nowrap">ชนิดแรม</Label>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center justify-between gap-2 text-sm">
            ...
            <Select value={sortMode} onValueChange={(v)=>onChangeSort(v as SortMode)}>
              <SelectTrigger className= text-sm"w-44"><SelectValue placeholder="เลือกการจัดเรียง"/></SelectTrigger>
              <SelectContent>
            ...
            <div key={cat} className="grid grid-cols-12 items-center gap-2 p-2 rounded-xl hover:bg-muted/40">
              ...
              <div className="col-span-8 flex gap-2">
                <Select value={selection[cat]?.id || "__none__"} onValueChange={(id)=>{ if(id==="__none__"){ onSelect(cat, null); if (required.includes(cat)) toast.error(`${cat} เป็นหมวดจำเป็น`); return; } const item = byCat[cat].find(p=>p.id===id) || null; onSelect(cat, item); }}>
                  <SelectTrigger className= text-sm"w-full"><SelectValue placeholder={`เลือก ${cat} (ข้ามได้)`} /></SelectTrigger>
                  <SelectContent>
      ...
      <Card className="shadow-sm">
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between mt-3 text-sm text-muted-foreground"><span>ประมาณการกำลังไฟ ~ {estimateWattage(selection)}W</span><Button  variant="secondary" onClick={()= className="h-8 px-3 text-sm">setShowAllComp(s=>!s)}>{showAllComp ? "ย่อ" : "ดูทั้งหมด"}</Button></div>
          </div>
        </CardContent>
      </Card>
    </div>
        ...
        <div className="grid grid-cols-12 items-center gap-2 p-2 rounded-xl hover:bg-muted/40">
          ...
          <div className="col-span-6">
            <Select value={selMonitor} onValueChange={setSelMonitor}>
              <SelectTrigger className= text-sm"w-full"><SelectValue placeholder="เลือก Monitor" /></SelectTrigger>
              <SelectContent>
        ...
        <div className="grid grid-cols-12 items-center gap-2 p-2 rounded-xl hover:bg-muted/40">
          ...
          <div className="col-span-6">
            <Select value={selSoftware} onValueChange={setSelSoftware}>
              <SelectTrigger className= text-sm"w-full"><SelectValue placeholder="เลือก Software" /></SelectTrigger>
              <SelectContent>
        ...
        <div className="grid grid-cols-12 items-center gap-2 p-2 rounded-xl hover:bg-muted/40">
          ...
          <div className="col-span-6">
            <Select value={selSSD} onValueChange={setSelSSD}>
              <SelectTrigger className= text-sm"w-full"><SelectValue placeholder="เลือก SSD" /></SelectTrigger>
              <SelectContent>
        ...
        <div className="grid md:grid-cols-2 gap-3 p-3 rounded-xl bg-muted/40">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={pricing.vatEnabled ? "on" : "off"} onValueChange={(v)=>setPricing({ ...pricing, vatEnabled: v==="on" })}>
              <SelectTrigger className= text-sm"w-28"><SelectValue /></SelectTrigger>
...
export default function App(){
  ...
  useEffect(() => {
    document.title = "Ubon Computer Spec V.3.1";
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <Tabs defaultValue="builder" className="space-y-4">
            ...
            <CardContent className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {BASE_CATEGORIES.map(c=>{
                  ...
                  return <Button key={c} className="h-8 px-3 text-xs md:text-sm whitespace-nowrap" variant="secondary" onClick={()=>{
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button className="h-8 px-3 text-xs md:text-sm whitespace-nowrap" variant="secondary" onClick={()=>{ setSmartSync(!smartSync); toast.message(!smartSync ? 'เปิด Smart Sync' : 'ปิด Smart Sync'); }}>{smartSync ? "เปิดอยู่" : "ปิดอยู่"}</Button>
                <Button className="h-8 px-3 text-xs md:text-sm whitespace-nowrap" variant="secondary" onClick={()=>{ setFilters(deriveFiltersFromSelection(build.base, {})); toast.message('ซิงก์ตัวกรองแล้ว'); }}>ซิงก์ตอนนี้</Button>
              </div>
        ...
        <TabsContent value="inventory" className="space-y-4">
          <Card className="shadow-sm">
            <CardHeader><CardTitle>คลังสินค้า</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                <Input placeholder="ค้นหา..." value={query} onChange={(e)=>setQuery(e.target.value)} className="w-48"/>
                <Select value={sortMode} onValueChange={(v)=>setSortMode(v as SortMode)}>
                  <SelectTrigger className= text-sm"w-48"><SelectValue placeholder="จัดเรียง" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="default">ค่าเริ่มต้น</SelectItem>
                    <SelectItem value="priceAsc">ราคาต่ำ→สูง</SelectItem>
                    <SelectItem value="priceDesc">ราคาสูง→ต่ำ</SelectItem>
                ...
                <div className="ml-auto flex items-center gap-2">
                  ...
                  <Button variant="secondary" onClick={()=>fileRef.current?.click()}><FileUp className="w-4 h-4 mr-2"/> นำเข้า Excel/CSV</Button>
                  <Button onClick={()=>setEditing({} as Product)}><Plus className="w-4 h-4 mr-2"/> เพิ่มสินค้า</Button>
                  <Button variant="destructive" onClick={clearAll}><Trash2 className="w-4 h-4 mr-2"/> รีเซ็ตข้อมูลตัวอย่าง</Button>
                </div>
              </div>
            ...
            <div className="border rounded-xl max-h-80 overflow-auto">
              {(() => {
                ...
                const results = sortProducts(
                  inventory.filter(p => !q || [p.name, p.category, JSON.stringify(p.attributes)].join(" ").toLowerCase().includes(q)),
                  sortMode
                );
                  ...
                  <div key={p.id} className="flex items-center justify-between px-4 py-2 border-t first:border-t-0 hover:bg-muted/40">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {((BASE_CATEGORIES as readonly string[]).includes(p.category)) ? (
                        <Button onClick={()=>{ selectBase(p.category as BaseCategory, p); toast.success(`ใส่ ${p.category} แล้ว`); setSearchOpen(false); }}>ใส่ใน {p.category}</Button>
                      ) : (
                        <Button onClick={()=>{ addAddon(p, 1); toast.success(`เพิ่ม ${p.name} (Option)`); setSearchOpen(false); }}>เพิ่มเป็น Option</Button>
                      )}
                    </div>
                  </div>
                ));
